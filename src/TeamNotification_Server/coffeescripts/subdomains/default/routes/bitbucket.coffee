...
methods.bitbucket_repositories = (req,res)->
    ...
    callback = (collection) ->
        ...
        add_user_data_to_collection(req.user, collection.to_json()).then (json) ->
            res.json(json)
    build('bitbucket_repositories_collection').for({oa:oa, oauth_access_token:req.param("access_token"), oauth_access_token_secret:req.query.oauth_access_token_secret, user_id:req.user.id} ).fetch_to callback
