...
is_valid_user = (user_data) ->
    ...
    validator.check(user_data.id, 'Id must be provided').notEmpty()
    ...
    if user_data.password isnt ''
        validator.check(user_data.password, 'Password must contain at least 6 characters').len(6, 48)
