...
map = (event_obj) ->
    ...
    return {
        ...
        message: "Github notification! User, #{event_obj.comment.user.login}, just #{event_obj.action}ed issue '#{event_obj.issue.title}' on repository: #{event_obj.repository.name} {0} - {1}",
        notification:1,
