...
(function() {
  var Q, clean_up_db, create_db_tables, creation_promise, db_config, gateway, insert_entity, relate, set_up_db, sync, _entity;
  ...
  clean_up_db = function() {
    return Q.when(creation_promise, function() {
      return gateway.open(db_config.test).then(function(db) {
        var entity, val, _results;
        _results = [];
        for (entity in _entity) {
          val = _entity[entity];
          _results.push(_entity[entity].clear());
        }
        return _results;
      });
    });
