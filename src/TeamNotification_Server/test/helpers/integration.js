  ...
  clean_up_db = function() {
    ...
    return Q.when(creation_promise, function() {
      return pg_gateway.open(db_config.test).then(function(client) {
        return client.query("SELECT 'drop table if exists ' || tablename || ' cascade;' as drop_statement FROM pg_tables;", function(err, result) {
          ...
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            row = _ref[_i];
            _results.push(client.query(row.drop_statement, drop_callback));
          }
          return _results;
        });
      });
