...
describe 'Users Collection', ->
    ...
    describe 'to_json', ->
        ...
        it 'should return the users collection users in the resulting json', (done) ->
            data = result['users']
            expect(data[0]).to.eql {"href": "/user/#{users[0].id}", "data": [{"name": "id", "value": users[0].id}, {"name": "name", "value": users[0].name}]}
            expect(data[1]).to.eql {"href": "/user/#{users[1].id}", "data": [{"name": "id", "value": users[1].id}, {"name": "name", "value": users[1].name}]}
            done()
